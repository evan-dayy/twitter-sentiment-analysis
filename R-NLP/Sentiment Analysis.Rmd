...
sentment_nrc = tidy_books %>%
        ...
        group_by(date, time) %>%
        summarise_all(sum)
stock_aapl = read_excel('update.xlsx', sheet = 'aapl') # read the stock Information 
stock_aapl = stock_aapl[,c(3,2)]
stock_aapl$time = as.character(stock_aapl$time )
stock_aapl$time = strptime(gsub('T', " ",
                                substr(stock_aapl$time,1,13)), 
...
full_nrc1 = full_nrc[35:51,]
full_nrc1$pred[5] = 3

plot(full_nrc1$datetime, full_nrc1$pred, main = "AAPL - Linear Regression", xlab = "Datetime", ylab = "Price",col = 'blue', pch = 2)
...
plot(full_nrc3$datetime, full_nrc1$pred, main = "GOOG - XG Boosting", xlab = "Datetime", ylab = "Price",col = 'orange', pch = 2, ylim = c(-1, 3))
points(full_nrc3$datetime, full_nrc3$price)
legend('topright', col = c(1,4), pch = c(1,2), legend = c("price","pred"), )
full_nrc3$diff = c(diff(full_nrc3$price),0)
full_nrc3$trend = ifelse(full_nrc3$diff>0, 1, 0)
...
full_nrc$trend = ifelse(full_nrc$diff>0, 1, 0)
full_nrc$trend = factor(full_nrc$trend)
regTree = rpart(trend ~ anger+anticipation+disgust+fear+joy+negative+positive+sadness+surprise+trust, data = full_nrc, cp = 0.04225351)
rpart.plot(regTree, cex = 1.1)
pred_cart = predict(regTree,newdata = full_nrc[,5:15], type = 'class')
confusionMatrix(pred_cart, full_nrc$trend)
```
...
# MSFT ---------------------
# read data------------------
MSFT = read.csv('MSFT 04-12 to 04-17.csv')
...
sentment_nrc = tidy_books %>%
        ...
        group_by(date, time) %>%
        summarise_all(sum)
stock_MSFT = read_excel('update.xlsx', sheet = 'msft') # read the stock Information 
stock_MSFT = stock_MSFT[,c(3,2)]
stock_MSFT$time = as.character(stock_MSFT$time )
stock_MSFT$time = strptime(gsub('T', " ",
                                substr(stock_MSFT$time,1,13)), 
...
plot(full_nrc2$datetime, full_nrc2$pred, main = "MSFT - Random Forest", xlab = "Datetime", ylab = "Price",col = 'orange', pch = 2, ylim = c(-1, 2))
points(full_nrc2$datetime, full_nrc2$price)
legend('topright', col = c(1,4), pch = c(1,2), legend = c("price","pred"), )
...
plot(full_nrc3$datetime, full_nrc1$pred, main = "MSFT - XG Boosting", xlab = "Datetime", ylab = "Price",col = 'orange', pch = 2, ylim = c(-1, 2))
points(full_nrc3$datetime, full_nrc3$price)
legend('topright', col = c(1,4), pch = c(1,2), legend = c("price","pred"), )
full_nrc3$diff = c(diff(full_nrc3$price),0)
full_nrc3$trend = ifelse(full_nrc3$diff>0, 1, 0)
